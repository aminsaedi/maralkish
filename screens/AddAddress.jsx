...
import { apiGetUserProfile, apiAddNewAddress } from "../api/users";
...
const AddAddress = ({ navigation, route }) => {
  ...
  const handleAddAddress = async (values) => {
    ...
    try {
      const result = await apiAddNewAddress({
      ...
      if (result.status === 200) {
        if (route.params && route.params.returnTo) {
          return navigation.push(route.params.returnTo);
        }
        navigation.navigate(pages.myAccount);
      }
    } catch (error) {
      setButtonLoading(false);
        ...
        <AppView isSAfe>
          ...
          >
            ...
            <TextInput
              ...
              onEndEditing={handleGetDirectAddresses}
            />
